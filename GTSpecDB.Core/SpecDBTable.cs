...
namespace GTSpecDB.Core
{
    public class SpecDBTable
    {
        ...
        public ObservableCollection<SpecDBRowData> Rows { get; set; } = new ObservableCollection<SpecDBRowData>();
        ...
        public void AddressInitialize(SpecDB specDb)
        {
            ...
            if ((versionHigh & 1) != 0)
            {
                DBT.HuffmanTableEntryOffset = sr.Position + 0x08;
                DBT.SearchTableOffset = sr.Position + 0x208;

                sr.Position += sr.ReadInt32();
                DBT.DataMapHeaderOffset = sr.Position;
                DBT.DataMapOffset = sr.Position + 0x08;

                DBT.ShortTableOffset = sr.Position + sr.ReadInt32();
            }
            else
                DBT.ShortTableOffset = sr.Position;
        }
        ...
        private void SaveIDTable(string outputPath)
        {
            ...
            using (var bs = new BinaryStream(fs, BigEndian ? ByteConverter.Big : ByteConverter.Little))
            {
                ...
                var orderedRows = Rows
                    ...
                    .OrderBy(o => o.Label, new AlphanumericComparer()) // Mandatory sort for game bsearch
                    .ToList();
