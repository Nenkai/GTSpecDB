...
namespace GT_SpecDB_Editor.Core
{
    public class SpecDB
    {
        ...
        public async Task SavePartsInfo(ProgressWindow window, IProgress<(int, string)> progress, string folder)
        {
            try
            {
                await Task.Run(() =>
                {
                    ...
                    using (var tbiWriter = new BinaryStream(new FileStream(Path.Combine(folder, "PartsInfo.tbi"), FileMode.Create)))
                    {
                        ...
                        tbiWriter.WriteInt32(Version);
                    }
                });
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Could not save Parts Info: {ex.Message}", "Failed to save parts info", MessageBoxButton.OK, MessageBoxImage.Error);
            }
